...
import com.general.carparking.vehicle.VehicleSpecification;
...
public class VehicleParkingService implements IVehicleParkingService {
    ...
    @Override
    public List<VehicleParking> search(List<SearchCriteria> searchCriteria) {
        VehicleParkingSpecification vehicleParkingSpecification=new VehicleParkingSpecification();

        searchCriteria.forEach(criteria -> vehicleParkingSpecification.add(criteria));
System.out.println(searchCriteria);
         return repository.findAll(vehicleParkingSpecification);
    }


    @Override
    public Double getAllByVehicleCarPlaque(String carPlaque) {
     /*   Vehicle vehicle = vehicleService.search(carPlaque);
        List<VehicleParking> vehicleParkings = repository.findAllByVehicle(vehicle);
        return vehicleParkings;*/
        return null;
    }
